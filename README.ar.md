...
- **الالتقاط مرئيّ ويدويٌّ فعليًّا لكل جلسة** — يجري عبر متصفّح Chrome **مُسجَّل
  الدخول** (Claude-in-Chrome)، لا كاشط بلا واجهة. تُترَك الجلسة دون مساس (دون
  وضع خفيّ/تسجيل خروج/تبديل حساب)، لأنّ إجابة الذكاء الاصطناعي تعتمد على الحساب والإعداد المحلّي.
- **سطح الإجابة غير حتميّ** — قد يُعيد الاستعلام نفسه إجابة مختلفة أو
  لا شيء على الإطلاق. يلتقط open-geo ما عُرِض *الآن* ولا يُعيد المحاولة أملًا في إجابة
  «أفضل». الغياب **بيانات صحيحة** (`overview_present=false`) تُغذّي التغطية — لا فشل.
- **عمّال `--n-worker` يُشغَّلون بالتوازي.** تُقسَّم الاستعلامات إلى N أجزاء وتُشغَّل سُبأَجِنتات الالتقاط الـ N
  بالتزامن، كلٌّ في علامة تبويب/سياق متصفّح خاصّ به؛ `--n-worker` هو
  تزامُن الجولة.
- **خطر reCAPTCHA / «حركة مرور غير معتادة»** تحت الحِمل: عند التحدّي، يتوقّف الالتقاط ويطلب
  من الإنسان حلّه في نافذة Chrome المفتوحة بدلًا من إرهاق المحرّك.
- **منطقة رماديّة في شروط الخدمة** — أتمتة محرّك بحث/إجابة تقع في منطقة رماديّة من شروط خدمته.
  استخدم حسابًا **مُخصَّصًا**، وأبقِ الحجم منخفضًا، وعامِل هذا كأداة قياس، لا كاشطًا.
